...
El control del estado de Ethernet Switch (CSHM) utiliza SNMP para supervisar el estado y el rendimiento de los switches de red. Por defecto, SNMPv2c se configura automáticamente a través del Reference Configuration File (RCF). SNMPv3 es más seguro que SNMPv2 porque introduce funciones de seguridad sólidas como autenticación, cifrado e integridad de mensajes, que protegen contra el acceso no autorizado y aseguran la confidencialidad e integridad de los datos durante la transmisión.
