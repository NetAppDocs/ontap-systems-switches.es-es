...
summary: 'Puede utilizar este procedimiento para configurar SNMPv3 para su switch específico, que admite la monitorización del estado del switch Ethernet (CSHM).' 
---
= Configure SNMPv3 para su switch (opcional)
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
SNMP se utiliza para monitorizar los switches.  La monitorización mediante SNMPv3 se configura siguiendo este procedimiento.

El monitor de estado del conmutador Ethernet (CSHM) utiliza SNMP para supervisar el estado y el rendimiento de los conmutadores de clúster y de almacenamiento.  Por defecto, SNMPv2c se configura automáticamente a través del archivo de configuración de referencia (RCF).  SNMPv3 es más seguro que SNMPv2 porque introduce características de seguridad robustas como la autenticación, el cifrado y la integridad de los mensajes, que protegen contra el acceso no autorizado y garantizan la confidencialidad e integridad de los datos durante la transmisión.

[NOTE]
====
* SNMPv3 solo es compatible con ONTAP 9.12.1 y versiones posteriores.
* Las versiones ONTAP 9.13.1P12, 9.14.1P9, 9.15.1P5, 9.16.1 y posteriores solucionan estos dos problemas:
+
** link:https://kb.netapp.com/on-prem/ontap/OHW/OHW-Issues/CONTAP-82891["Para la monitorización del estado de los switches Cisco ONTAP , es posible que aún se observe tráfico SNMPv2 después de cambiar a SNMPv3 para la monitorización."^]
** link:https://kb.netapp.com/on-prem/ontap/OHW/OHW-Issues/CONTAP-119131["Alertas falsas positivas de ventilador y alimentación cuando se producen fallos SNMP"^]
...
Los siguientes comandos se utilizan para configurar un nombre de usuario SNMPv3 en switches *Broadcom*, * Cisco* y * NVIDIA*:

[role="tabbed-block"]
====
.conmutadores Broadcom
--
Configure un nombre de usuario SNMPv3 NETWORK-OPERATOR en los switches Broadcom BES-53248.
...
----
. Tras esperar el período de sondeo CSHM, verifique que el número de serie esté registrado para el conmutador Ethernet.
+
[source, cli]
----
...
.conmutadores Cisco
--
Configure un nombre de usuario SNMPv3_USER en los switches Cisco 9336C-FX2:
...
.NVIDIA - CL 5.4.0
--
Configure un nombre de usuario SNMPv3_USER en switches NVIDIA SN2100 con CLI 5.4.0:
...
.NVIDIA - CL 5.11.0
--
Configure un nombre de usuario SNMPv3_USER en switches NVIDIA SN2100 con CLI 5.11.0:
