...
[NOTE]
====
. Vuelva a generar las teclas ssh en el interruptor, según las instrucciones del proveedor.
. Regenerar las teclas ssh en el lado de ONTAP mediante `debug system regenerate-systemshell-key-pair`
. Volver a ejecutar la rutina de configuración de la recopilación de registros mediante `system switch ethernet log setup-password`
...
* Para los switches NVIDIA, el usuario para la recopilación de registros debe tener permiso para ejecutar los comandos de recopilación de registros sin mostrar una solicitud de contraseña. Para permitir este uso, ejecute el comando:
