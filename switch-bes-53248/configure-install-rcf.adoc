...
summary: Puedes instalar el archivo de configuración de referencia RCF después de configurar el conmutador de clúster BES-53248 y después de aplicar las nuevas licencias. 
---
= Instala el archivo de configuración de referencia (RCF)
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Puedes instalar el RCF después de configurar el switch de clúster BES-53248 y después de aplicar las nuevas licencias.


CAUTION: Para EFOS 3.12 y posteriores, sigue los pasos de instalación en link:efos-install-rcf-license-file.html["Instala el archivo de configuración de referencia (RCF) y el archivo de licencia"].
...
* El archivo RCF actual, disponible en la link:https://mysupport.netapp.com/site/products/all/details/broadcom-cluster-switches/downloads-tab["Conmutadores de clúster Broadcom"^] página.
* Una configuración de arranque en el RCF que refleje las imágenes de arranque deseadas, necesaria si solo va a instalar EFOS y mantener su versión actual de RCF.  Si necesita cambiar la configuración de arranque para que refleje las imágenes de arranque actuales, debe hacerlo antes de volver a aplicar el RCF para que se instancie la versión correcta en futuros reinicios.
* Una conexión de consola al switch, necesaria cuando se instala el RCF desde un estado predeterminado de fábrica. Este requisito es opcional si has usado el artículo de base de conocimientos link:https://kb.netapp.com/onprem/Switches/Broadcom/How_to_clear_configuration_on_a_Broadcom_interconnect_switch_while_retaining_remote_connectivity["Cómo borrar la configuración de un switch de interconexión Broadcom manteniendo la conectividad remota"^] para borrar la configuración antes.


.Documentación sugerida
Consulta la tabla de compatibilidad de switches para las versiones compatibles de ONTAP y RCF. Consulta link:https://mysupport.netapp.com/site/info/broadcom-cluster-switch["Descarga del software EFOS"^]. Ten en cuenta que puede haber dependencias de comandos entre la sintaxis de comandos en el RCF y la que se encuentra en las versiones de EFOS.
...
* Los nombres de los dos conmutadores BES-53248 son cs1 y cs2.
* Los nombres de los nodos son cluster1-01, cluster1-02, cluster1-03 y cluster1-04.
* Los nombres del clúster LIF son cluster1-01_clus1, cluster1-01_clus2, cluster1-02_clus1, cluster1-02_clus2, cluster1-03_clus1, cluster1-03_clus2, cluster1-04_clus1 y cluster1-04_clus2.
* El `cluster1::*>` El indicador muestra el nombre del clúster.
* Los ejemplos de este procedimiento utilizan cuatro nodos. Estos nodos utilizan dos puertos de interconexión de clúster 10GbE `e0a` y `e0b`. Consulta el link:https://hwu.netapp.com/Home/Index["Hardware Universe"^] para verificar los puertos de clúster correctos en tus plataformas.
...
El procedimiento requiere el uso de comandos ONTAP y comandos de conmutador Broadcom; se utilizan comandos ONTAP a menos que se indique lo contrario.
...
CAUTION: Antes de instalar una nueva versión del software del switch y los RCFs, usa el artículo de base de conocimientos link:https://kb.netapp.com/onprem/Switches/Broadcom/How_to_clear_configuration_on_a_Broadcom_interconnect_switch_while_retaining_remote_connectivity["Cómo borrar la configuración de un switch de interconexión Broadcom manteniendo la conectividad remota"^]. Si tienes que borrar completamente la configuración del switch, entonces necesitas realizar la configuración básica de nuevo. Debes estar conectado al switch usando la consola serie porque un borrado completo de la configuración restablece la configuración de la red de gestión.
...
--
A partir de ONTAP 9.8, utilice el siguiente comando:
...
. En el switch cs2, confirme la lista de puertos que están conectados a los nodos del clúster.
+
[source, cli]
----
show isdp neighbor
----
. En el conmutador de clúster cs2, apague los puertos conectados a los puertos de clúster de los nodos.  Por ejemplo, si los puertos 0/1 a 0/16 están conectados a nodos ONTAP :
+
[listing, subs="+quotes"]
----
(cs2)> *enable*
...
(cs2)(Config)#
----
. Verifique que las LIF del clúster se hayan migrado a los puertos alojados en el conmutador del clúster cs1. Esto podría tardar unos segundos.
+
[source, cli]
----
network interface show -vserver Cluster
...
----
====
. Si aún no lo ha hecho, guarde la configuración actual del switch copiando el resultado del siguiente comando en un archivo de registro:
+
[source, cli]
----
show running-config
...
CAUTION: Al actualizar o aplicar un nuevo RCF, debe borrar la configuración del switch y realizar la configuración básica.  Para borrar la configuración del switch, debe estar conectado a él mediante la consola serie.  Este requisito es opcional si ha utilizado el artículo de la base de conocimientos. https://kb.netapp.com/onprem/Switches/Broadcom/How_to_clear_configuration_on_a_Broadcom_interconnect_switch_while_retaining_remote_connectivity["Cómo borrar la configuración de un switch de interconexión Broadcom manteniendo la conectividad remota"] Para borrar la configuración de antemano.

+

NOTE: Borrar la configuración no elimina las licencias.

+
.. Conéctese al switch mediante SSH.
+
Proceda únicamente cuando se hayan retirado todos los LIF del clúster de los puertos del conmutador y este esté preparado para que se borre la configuración.
...
(cs2)#
----
.. Copie y pegue los siguientes comandos para eliminar la configuración RCF anterior (dependiendo de la versión RCF anterior utilizada, algunos comandos podrían generar un error si falta alguna configuración):
+
[source, cli]
----
...
----
.. Vuelva a iniciar sesión en el switch mediante SSH para completar la instalación de RCF.


. Tenga en cuenta lo siguiente:
+
.. Si se han instalado licencias de puerto adicionales en el switch, debes modificar el RCF para configurar los puertos adicionales con licencia. Consulta link:configure-licenses.html#activate-newly-licensed-ports["Activar los puertos recién licenciados"] para más detalles.
.. Registre cualquier personalización realizada en el RCF anterior y aplíquela al nuevo RCF.  Por ejemplo, configurar las velocidades de los puertos o codificar el modo FEC.
...
[role="tabbed-block"]
====
.EFOS versión 3.12.x y posteriores
--
. Copie el RCF al bootflash del conmutador cs2 utilizando uno de los siguientes protocolos de transferencia: HTTP, HTTPS, FTP, TFTP, SFTP o SCP.
+
Este ejemplo muestra cómo se utiliza SFTP para copiar un RCF a la memoria flash de arranque del switch cs2:
...
--
.Todas las demás versiones de EFOS
--
. Copie el RCF al bootflash del conmutador cs2 utilizando uno de los siguientes protocolos de transferencia: HTTP, HTTPS, FTP, TFTP, SFTP o SCP.
+
Este ejemplo muestra cómo se utiliza SFTP para copiar un RCF a la memoria flash de arranque del switch cs2:
...
--
====
. Examine la salida del banner de `show clibanner` dominio.  Debe leer y seguir estas instrucciones para verificar la configuración y el funcionamiento correctos del interruptor.
+
.Mostrar ejemplo
[%collapsible]
====
...
----
====
. En el switch, verifique que los puertos con licencia adicionales aparezcan después de aplicar el RCF:
+
[source, cli]
----
...
----
====
. Verifique en el switch que los cambios se hayan realizado:
+
[source, cli]
----
show running-config
...
----
. En el conmutador de clúster cs2, active los puertos conectados a los puertos de clúster de los nodos.  Por ejemplo, si los puertos 0/1 a 0/16 están conectados a nodos ONTAP :
+
[listing, subs="+quotes"]
----
(cs2)> *enable*
...
(cs2)(Config)#
----
. Verifique los puertos del switch cs2:
+
[source, cli]
----
...
+
.. Verifique que los puertos e0b estén activos y en buen estado en todos los nodos del clúster:
+
[source, cli]
----
network port show -ipspace Cluster
...
--
====
. [[step20]]En el conmutador de clúster cs1, apague los puertos conectados a los puertos de clúster de los nodos.
+
El siguiente ejemplo utiliza la salida de ejemplo de la interfaz:
...
----
. Reiniciar interruptor cs1.  Esto provoca que los LIF del clúster vuelvan a sus puertos de origen.  Puede ignorar los eventos de "puertos del clúster inactivos" que se reportan en los nodos mientras se reinicia el conmutador.
+
[listing, subs="+quotes"]
----
(cs1)# *reload*
...
. En el switch cs1, verifique que los puertos del switch conectados a los puertos del clúster estén *activos*:
+
[source, cli]
----
show interfaces status all | exclude Detach
...
----
====
. Verifique que el enlace ISL entre los conmutadores cs1 y cs2 funciona correctamente:
+
[source, cli]
----
...
.¿Que sigue?
Una vez instalado RCF, puedes link:configure-ssh.html["habilitar SSH"].
