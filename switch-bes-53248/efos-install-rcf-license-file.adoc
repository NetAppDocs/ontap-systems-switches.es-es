...
[role="lead"]
A partir de EFOS 3.12.0.1, puede instalar el archivo de configuración de referencia (RCF) y el archivo de licencia después de configurar el conmutador de cluster BES-53248.


CAUTION: Todos los puertos se configuran al instalar el RCF, pero es necesario instalar la licencia para activar los puertos configurados.
...
* El RCF actual, disponible en la https://mysupport.netapp.com/site/products/all/details/broadcom-cluster-switches/downloads-tab["Switches de clúster Broadcom"^] página.
...
. Copie el RCF en el bootflash del switch CS2 usando uno de los siguientes protocolos de transferencia: FTP, HTTP, TFTP, SFTP o SCP.
+
Este ejemplo muestra HTTP que se está utilizando para copiar un RCF al bootflash en el switch CS2:
...
----
====
. En el interruptor, compruebe que se han realizado los cambios:
+
[source, cli]
----
show running-config
...
--
====
. [[step21]]En el switch de cluster CS1, apague los puertos conectados a los puertos de cluster de los nodos.
+
