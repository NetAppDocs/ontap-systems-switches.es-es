...
* Una conexión de consola al switch, necesaria cuando se instala el RCF desde un estado predeterminado de fábrica.
...
Usas la CLI del switch y la CLI de ONTAP (como se indica en los siguientes pasos) para las configuraciones de puertos. Configuras los puertos en el primer switch (cs2) y luego repites estos pasos para el segundo switch (cs1).
...
CAUTION: Al actualizar o aplicar un nuevo RCF, debes borrar la configuración del conmutador y realizar la configuración básica. Debes estar conectado al conmutador mediante la consola serie para borrar la configuración del conmutador.
...
--
====
. [[step20]]Repite los pasos 1 a 19 en el switch cs1.
...
--
====
. [[step6]] Cambia el nivel de privilegio de nuevo a admin:
+
[source, cli]
----
set -privilege admin
...
.¿Que sigue?
Una vez que hayas actualizado tu RCF, puedes link:configure-ssh.html["activar SSH para la recopilación de registros"].
