...
summary: Puedes actualizar el RCF después de configurar el switch de clúster BES-53248 y después de aplicar cualquier licencia nueva. 
---
= Actualiza la versión de RCF
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Puedes actualizar el RCF después de actualizar el EFOS del switch de clúster BES-53248 y después de aplicar cualquier clave de licencia nueva.
...
* El archivo RCF actual, disponible en el https://mysupport.netapp.com/site/products/all/details/broadcom-cluster-switches/downloads-tab["Conmutadores de clúster Broadcom"^] página.
...
* Se requiere una conexión de consola al switch al instalar el RCF desde un estado predeterminado de fábrica.  Este requisito es opcional si ha utilizado el artículo de la base de conocimientos. https://kb.netapp.com/onprem/Switches/Broadcom/How_to_clear_configuration_on_a_Broadcom_interconnect_switch_while_retaining_remote_connectivity["Cómo borrar la configuración de un switch de interconexión Broadcom manteniendo la conectividad remota"^] Para borrar la configuración de antemano.
...
* Consulta las guías de software y actualización correspondientes disponibles en el sitio link:https://www.broadcom.com/support/bes-switch["Broadcom"^] para obtener la documentación completa sobre los procedimientos de actualización y downgrade del switch BES-53248.
...
CAUTION: Antes de instalar una nueva versión del software del switch y los RCF, consulte el artículo de la base de conocimientos. https://kb.netapp.com/onprem/Switches/Broadcom/How_to_clear_configuration_on_a_Broadcom_interconnect_switch_while_retaining_remote_connectivity["Cómo borrar la configuración de un switch de interconexión Broadcom manteniendo la conectividad remota"^] .  Si necesita borrar por completo la configuración del interruptor, deberá realizar de nuevo la configuración básica.  Debe conectarse al switch mediante la consola serie, ya que un borrado completo de la configuración restablece la configuración de la red de administración.



== Paso 1: Prepararse para la actualización
...
show isdp neighbor
----
. En el switch cs2, apague los puertos conectados a los puertos del clúster de los nodos.  Por ejemplo, si los puertos 0/1 a 0/16 están conectados a nodos ONTAP :
+
[listing, subs="+quotes"]
----
(cs2)> *enable*
...
+
.. Si se han instalado licencias de puerto adicionales en el switch, debes modificar el RCF para configurar los puertos adicionales con licencia. Consulta link:configure-licenses.html#activate-newly-licensed-ports["Activar los puertos recién licenciados"] para más detalles. Sin embargo, cuando actualizas a RCF 1.12 o posterior, ya no necesitas hacer modificaciones porque ahora todas las interfaces están preconfiguradas.
...
CAUTION: El archivo debe llamarse `reference-config.scr` cuando subas y apliques el archivo RCF, de lo contrario, el paso de aplicar el script fallará.

. Copia el RCF en el switch cs2 utilizando uno de los siguientes protocolos de transferencia: HTTP, HTTPS, FTP, SFTP o SCP.
+
...
--
====
. [[step11]]Examine la salida del banner del `show clibanner` dominio. Debe leer y seguir estas instrucciones para garantizar la configuración y el funcionamiento adecuados del conmutador.
+
`show clibanner`
...
----
====
. Comprueba en el switch que se hayan realizado los cambios.
+
[source, cli]
----
show running-config
...
----
. En el conmutador de clúster cs2, active los puertos conectados a los puertos de clúster de los nodos.
+
[listing, subs="+quotes"]
----
(cs2)> *enable*
...
----
+
Para más detalles, consulta link:https://docs.netapp.com/us-en/ontap/networking/revert_a_lif_to_its_home_port.html["Revierte un LIF a su puerto de origen"].
