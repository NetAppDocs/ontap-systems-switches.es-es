...
summary: Puede instalar el RCF después de configurar el conmutador Nexus 3132Q-V por primera vez. También puede utilizar este procedimiento para actualizar su versión de RCF. 
---
...
[role="lead"]
Siga este procedimiento para instalar el RCF después de configurar el switch Nexus 3132Q-V por primera vez. También puede utilizar este procedimiento para actualizar su versión de RCF.
...
* El archivo de configuración de referencia (RCF) actual.
* Se requiere una conexión de consola al switch al instalar el RCF.  Este requisito es opcional si ha utilizado el artículo de la base de conocimientos. https://kb.netapp.com/on-prem/Switches/Cisco-KBs/How_to_clear_configuration_on_a_Cisco_interconnect_switch_while_retaining_remote_connectivity["Cómo borrar la configuración de un switch de interconexión Cisco manteniendo la conectividad remota"^] Para borrar la configuración de antemano.
* link:https://mysupport.netapp.com/site/info/cisco-ethernet-switch["conmutador Ethernet de Cisco"^]. Consulte la tabla de compatibilidad del switch para conocer las versiones compatibles de ONTAP y RCF.  Tenga en cuenta que puede haber dependencias de comandos entre la sintaxis de comandos en RCF y la que se encuentra en las versiones de NX-OS.
* https://www.cisco.com/c/en/us/support/switches/nexus-3000-series-switches/products-installation-guides-list.html["Switches Cisco Nexus serie 3000"^]. Consulte las guías de software y actualización correspondientes disponibles en el sitio web de Cisco para obtener documentación completa sobre los procedimientos de actualización y degradación de los switches de Cisco .
...
* Los nombres de los clústeres LIF son `cluster1-01_clus1` , `cluster1-01_clus2` , `cluster1-02_clus1` , `cluster1-02_clus2` , `cluster1-03_clus1` , `cluster1-03_clus2` , `cluster1-04_clus1` , y `cluster1-04_clus2` .
...
Asegúrese de completar el procedimiento enlink:prepare-install-cisco-nexus-3132q.html["Prepárese para instalar el software NX-OS y el archivo de configuración de referencia."] y luego siga los pasos a continuación.



=== Paso 1: Comprobar el estado del puerto
...
----
====
. Examine la salida del banner de `show banner motd` dominio.  Debe leer y seguir las instrucciones que figuran en *Notas importantes* para garantizar la configuración y el funcionamiento correctos del conmutador.
+
.Mostrar ejemplo
[%collapsible]
====
...
NOTE: Para conocer los pasos para poner en línea los puertos 10GbE después de una actualización del RCF, consulte el artículo de la base de conocimientos. https://kb.netapp.com/onprem%2FSwitches%2FCisco%2F10GbE_ports_on_Cisco_3132Q_cluster_switch_do_not_come_online["Los puertos 10GbE de un switch de clúster Cisco 3132Q no se activan."^] .
...
----
====
. Aplique el mismo RCF y guarde la configuración en ejecución por segunda vez.  Esto es necesario ya que el RCF utiliza QoS y requiere una reconfiguración de TCAM que implica cargar el RCF dos veces con el switch reiniciado entre ambas.
+
.Mostrar ejemplo
[%collapsible]
====
...
----
====
. Repita los pasos 1 a 17 en el interruptor cs1.
