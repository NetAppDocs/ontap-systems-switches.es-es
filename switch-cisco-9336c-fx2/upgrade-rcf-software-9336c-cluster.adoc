...
CAUTION: Asegúrate de apagar *todos* los puertos de clúster conectados para evitar cualquier problema de conexión de red. Consulta el artículo de base de conocimientos link:https://kb.netapp.com/on-prem/ontap/OHW/OHW-KBs/Node_out_of_quorum_when_migrating_cluster_lif_during_switch_OS_upgrade["Nodo fuera de quórum al migrar la LIF del clúster durante la actualización del sistema operativo del switch"^] para más detalles.
...
.. A partir de NX-OS 10.2, utilice el `show diff running-config` comando para comparar con el archivo RCF guardado en la memoria flash de arranque. De lo contrario, utilice una herramienta de comparación/diferenciación de terceros.
...
. Copia el RCF al bootflash en el switch cs1 usando uno de los siguientes protocolos de transferencia: FTP, SFTP o SCP.
+
Para obtener más información sobre los comandos de Cisco, consulta la guía correspondiente en el link:https://www.cisco.com/c/en/us/support/switches/nexus-9336c-fx2-switch/model.html#CommandReferences["Referencia de comandos de Cisco Nexus serie 9000 NX-OS"^].

+
Este ejemplo muestra SFTP siendo usado para copiar un RCF al bootflash en el switch cs1:
...
. Vuelva a aplicar cualquier personalización anterior a la configuración del switch.  Referirse alink:cabling-considerations-9336c-fx2.html["Revisar las consideraciones de cableado y configuración"] Para obtener detalles sobre cualquier otro cambio necesario.
...
----
====
. Repita los pasos 1 a 19 en el interruptor cs2.
. Habilitar la reversión automática en los LIF del clúster.
+
`cluster1::*> *network interface modify -vserver Cluster -lif * -auto-revert True*`

. Realiza un reinicio del switch cs2.
+
`cs2# *reload*`
...
CAUTION: Si tu conmutador 9336C-FX2 no arranca después de la actualización de RCF, consulta el artículo de base de conocimientos de NetApp link:https://kb.netapp.com/on-prem/ontap/OHW/OHW-KBs/Cisco_9336C-FX2_Switch_Fails_to_Boot_After_RCF_Install_%E2%80%93_Boot_Variable_Cleared["Cisco 9336C-FX2 Switch falla al arrancar después de instalar RCF – variable de arranque borrada"^] para conocer los próximos pasos.
