...
summary: Asegúrese de que lo siguiente esté disponible en el switch antes de instalar el script RCF 
---
= Instalar o actualizar el script del archivo de configuración de referencia (RCF)
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
* Cumulus Linux está instalado. Consulta la link:https://hwu.netapp.com/Switch/Index["Hardware Universe"^] para ver las versiones compatibles.
* Dirección IP, máscara de subred y puerta de enlace predeterminada definidas mediante DHCP o configuradas manualmente.



NOTE: Debe especificar un usuario en el RCF (además del usuario administrador) que se utilizará específicamente para la recopilación de registros.
...
| Grupo | En los puertos configurados para breakout 4x10GbE, un puerto está configurado para breakout 4x25GbE y los otros puertos están configurados para 40/100GbE. Admite tráfico de clúster/HA compartido en puertos para nodos que usan puertos de clúster/HA compartidos. Consulta la tabla de plataformas en el artículo de base de conocimientos link:https://kb.netapp.com/on-prem/ontap/OHW/OHW-KBs/What_AFF_ASA_and_FAS_platforms_use_shared_Cluster_and_HA_Ethernet_ports["¿Qué plataformas AFF, ASA y FAS utilizan puertos Ethernet de alta disponibilidad y de clúster compartidos?"^]. Todos los puertos pueden usarse también como puertos de clúster dedicados. 
...
Existen dos scripts RCF disponibles para aplicaciones de clúster y almacenamiento.  Descargar RCF desde https://mysupport.netapp.com/site/info/nvidia-cluster-switch["Descarga del software NVIDIA SN2100"^] página.  El procedimiento es el mismo para cada uno.

* Clúster: *MSN2100-RCF-v1._x_-Cluster-HA-Breakout-LLDP*
* Almacenamiento: *MSN2100-RCF-v1._x_-Almacenamiento*


.Acerca de los ejemplos
El siguiente procedimiento de ejemplo muestra cómo descargar y aplicar el script RCF para conmutadores de clúster.

La salida del comando de ejemplo utiliza la dirección IP de administración del switch 10.233.204.71, la máscara de red 255.255.254.0 y la puerta de enlace predeterminada 10.233.204.1.
...
. Utilice el `ping` comando para verificar la conectividad con el servidor que aloja Cumulus Linux y RCF.
...
----
. Copia el script de Python de RCF al switch.
+
[listing, subs="+quotes"]
----
...
NOTE: Mientras `scp` Si bien se utiliza en el ejemplo, puede usar su método preferido de transferencia de archivos, por ejemplo SFTP, HTTPS o FTP.

. Aplique el script de Python RCF *MSN2100-RCF-v1._x_-Cluster-HA-Breakout-LLDP*.
+
[listing, subs="+quotes"]
----
...
----
+
El script RCF completa los pasos enumerados en el ejemplo anterior.

+

NOTE: En el paso 3 *Actualización del archivo MOTD* anterior, el comando `cat /etc/motd` está en funcionamiento.  Esto le permite verificar el nombre del archivo RCF, la versión de RCF, los puertos a utilizar y otra información importante en el banner de RCF.
...
. Vuelva a aplicar cualquier personalización anterior a la configuración del switch.  Referirse alink:cabling-considerations-sn2100-cluster.html["Revisar las consideraciones de cableado y configuración"] Para obtener detalles sobre cualquier otro cambio necesario.
...
NOTE: En el paso 3 *Actualización del archivo MOTD* anterior, el comando `cat /etc/issue.net` está en funcionamiento.  Esto le permite verificar el nombre del archivo RCF, la versión de RCF, los puertos a utilizar y otra información importante en el banner de RCF.
...
NOTE: Las excepciones enumeradas no afectan al rendimiento y pueden ignorarse sin problema.
...
NOTE: A pesar de `scp` Si bien se utiliza en el ejemplo, puede usar su método preferido de transferencia de archivos, por ejemplo SFTP, HTTPS o FTP.
...
NOTE: En el paso 3 *Actualización del archivo MOTD* anterior, se ejecuta el comando `cat /etc/issue.net`.  Esto le permite verificar el nombre del archivo RCF, la versión de RCF, los puertos a utilizar y otra información importante en el banner de RCF.
...
====
.¿Que sigue?
Una vez instalado RCF, puedes link:setup-install-cshm-file.html["instalar el archivo CSHM"].
