...
---
= Actualizar el script del archivo de configuración de referencia (RCF)
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Siga este procedimiento para actualizar el script RCF.

.Antes de empezar
Antes de actualizar el script RCF, asegúrese de que lo siguiente esté disponible en el switch:
...
NOTE: A pesar de `scp` Si bien se utiliza en el ejemplo, puede utilizar su método preferido de transferencia de archivos.
...
NOTE: En el paso 3 *Actualización del archivo MOTD* anterior, el comando `cat /etc/issue` está en funcionamiento.  Esto le permite verificar el nombre del archivo RCF, la versión de RCF, los puertos a utilizar y otra información importante en el banner de RCF.
...
NOTE: Mientras `scp` Si bien se utiliza en el ejemplo, puede utilizar su método preferido de transferencia de archivos.
