...
* Para Cumulus Linux 5.11 y versiones posteriores, se utiliza el `nv action reset system factory-default` dominio.
...
NOTE: Para obtener más información sobre la instalación de Cumulus Linux, consultelink:configure-software-overview-sn2100-cluster.html["Flujo de trabajo de instalación de software para switches NVIDIA SN2100"] .
...
--
. Desde la consola de Cumulus, descargue y ponga en cola la instalación del software del switch con el comando `onie-install -a -i` seguido de la ruta de acceso al archivo del software del conmutador, por ejemplo:
+
[listing, subs="+quotes"]
----
...
NOTE: El conmutador se reinicia y accede a la instalación del software del conmutador, lo que lleva algún tiempo. Cuando finaliza la instalación, el switch se reinicia y permanece en el `log-in` inmediato.
...
Consulta la página de NVIDIA. https://docs.nvidia.com/networking-ethernet-software/cumulus-linux-511/Installation-Management/Factory-Reset/["Restablecimiento de fábrica"^] Documentación para más detalles.

--
====
.¿Qué sigue?
Después de reiniciar los interruptores, puedeslink:configure-overview-sn2100-cluster.html["reconfigurar"] los que necesiten.
